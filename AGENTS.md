...
Project context for Codex. Read this before proposing changes.
...
- **v15.15 is confirmed on live output, not only on fixtures.** The maintainer ran the extractor against real source material on 2026-08-29 and reports it working. That was the open question this release shipped with — the choice-run filter holds on genuine `pdfLayoutText` output, where a choice can wrap across lines, and not merely on the synthetic Davis-shaped fixture. Same pattern as every other real finding in this repo: fixtures proved the bug, live material proved the fix.
- **The mixed-marker hazard is still untested and remains the first place to look.** `nclexSplitByQNum` picks whichever pattern yields the most distinct numbers, so a book numbering stems `1.` and choices `1)` could let the choice pattern win on count while capturing only choice lines. The live run above does not clear this — it exercised one book's conventions, not that one. If a source ever extracts as short fragments, start here rather than with the choice-run filter.
