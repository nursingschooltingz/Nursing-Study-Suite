...
**v16.3 release:** [Download the single HTML file](https://github.com/nursingschooltingz/Nursing-Study-Suite/releases/download/v16.3/Nursing-Study-Suite.v16.3.html). Anki includes resumable source checks, precise citation diagnostics and an advisory review queue. Flash / Medium remains the default. Existing saved profiles stay intact; choose **Use recommended Anki profile** to update yours. Save the file, then open it in your browser. The [current repository HTML](Nursing-Study-Suite%20v16.3.html) contains the same application.
...
Use **Edit source links** inside that inspector to select supporting facts or enter their IDs, then apply the links. Changes retain a history and update current counts and diagnostics. The **Source-link review** filter identifies unmapped or disputed associations separately from overall linked-fact counts. **Condition-tag discrepancies** shows uncertain condition assignments. Only proven aliases with clean mappings normalize automatically; ambiguous names and source grouping need review.
...
In v16.3, malformed or truncated groups are listed separately while the check continues through other groups. **Retry / resume** checks only remaining groups using the original captured settings; accepted groups and original attempts are retained. A transport failure stops the session. Editing notes, changing the tier selection or replacing the KB requires a fresh packet. Small unambiguous citation formatting differences can be restored to the captured wording and are shown explicitly; unsupported text still fails.
...
The check compares captured fact text with actual recall targets. A completed group must include source-span target records for each selected primary fact and separate Text/Extra support records for each in-scope note. Expand **Checked targets and note fields** to inspect them. Findings can flag unsupported wording, changed meaning, missing targets, duplicates, priority loss and source conflicts, with source-supported suggestions where possible. Originals remain intact. These model judgments remain advisory; validated references do not certify complete coverage or correctness.

Preparation downloads an **unrun review packet**. After execution, **Download source-check report** records completed/incomplete/outdated status, profiles, timestamps, hashes, target records, findings and mapping history. Notes, selection, tier or source changes make old results outdated. Cancelled checks retain completed groups. Keep these downloads with your private course files.
