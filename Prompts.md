...
The full prompts behind the Knowledge Base builder, flashcard transcriber, Anki Generator, Priority Analyzer, NCLEX Extractor, NCLEX Generator, Case Study Generator, and the item-quality auditor, **extracted verbatim from the shipped v16.3 file** (spliced programmatically, not retyped — byte-identical to what the app sends).
...
The separate optional **Check against KB** flow uses `ankiBuildSourceAuditPrompt`, not the NCLEX item-audit prompt. Preparation captures bounded source/note packets and exact request strings locally; an explicit Run submits them using independently selected checker settings captured at preparation. Retry / resume reuses that capture for unaccepted groups. The auditor must return one target record for each selected primary fact, separate Text/Extra support records for each in-scope note, and advisory findings with source-supported suggestions. In v16.3, suggestions must preserve the three-cloze limit and selected higher-priority coverage, proposing separate notes for additional independent targets and avoiding answer-bearing hints. Literal source spans, note/cloze references, scope and receipt accounting are validated; a findings-only response is incomplete. Conservative whitespace/prose-capital recovery records both original and canonical citations. This validates declared evidence, not exhaustive target decomposition or semantic entailment. Originals remain unchanged. Its packets include the actual review fronts, hidden answers, Text and Extra; source quotes are excluded because they were not generation input.
