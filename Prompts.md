...
The full prompts behind the Knowledge Base builder, flashcard transcriber, Anki Generator, Priority Analyzer, NCLEX Extractor, NCLEX Generator, Case Study Generator, and the item-quality auditor, **extracted verbatim from the shipped v17.3 file** (spliced programmatically, not retyped — byte-identical to what the app sends).
...
The v16.6 runtime update supplies an exact canonical Condition token per source condition, preserves useful source-supported Extra while allocating substantive targets to hidden destinations, and checks for targets displaced during note splitting. Audit context locations may include both Text and Extra; repeated same-note disjoint indices normalize with recorded history. The checker is instructed to distinguish source-supported paraphrases from added propositions and independently important targets from necessary scaffold. Advisory local checks expose selected suspicious context classifications, short noun-only evidence and stronger qualifier wording, without changing model verdicts or proving meaning. All eleven frozen constants remain unchanged. Exact changes: [adapter](docs/history/ANKI-v16.6-adapter.diff), [serializer](docs/history/ANKI-v16.6-serializer.diff), [audit builder](docs/history/ANKI-v16.6-audit-builder.diff).
...
## Case item repair builder (v17.3)
