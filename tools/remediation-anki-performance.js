...
async function measure(browser,count,mode){
  ...
  await withFixture(browser,{indexed:syntheticKB('A')},async({page})=>{
    ...
    await page.waitForFunction(n=>document.querySelectorAll('.anki-tbl-wrap tbody tr').length===n,count,{timeout:45000});
    await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
    result=await page.evaluate(({count,mode,begin})=>({count,mode,renderMs:Math.round(performance.now()-begin),domElements:document.querySelectorAll('*').length,
...
module.exports={syntheticNotes,measure};
