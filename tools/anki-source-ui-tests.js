...
async function runAnkiSourceUiTests({ S, t, section }) {
  ...
  const capture = new Function('state', 'services', `
    const {cards,batch,tierFilter,current,busy,auditBusy,sourceAudit,model,toolLevel,cfg}=state;
  ...
  const create = (transport, changes = {}) => {
    ...
    const state = { cards, batch: { snapshot: { facts }, chunkIds: [['fact-1'], ['fact-2']] }, tierFilter: 'all', current: true, busy: false, auditBusy: false, sourceAudit: null, model: 'synthetic-model', toolLevel: 'low', cfg: { apiKey: credential }, ...changes };
  ...
  const happy = create(async (_key, _model, _parts, options) => { options.onMeta({ usage: { totalTokenCount: 11 } }); return clean; });
  ...
  t('explicit Run checks every prepared group with the captured model and thinking level', happy.calls.length === 2 && happy.calls.every(c => c[1] === 'synthetic-model' && c[3].thinkingLevel === 'low'));
  ...
  const stalePrepared = create(async () => clean); stalePrepared.prepare(); stalePrepared.state.cards = [...stalePrepared.state.cards];
  ...
  const noKey = create(async () => clean, { cfg: { apiKey: '' } });
  ...
  const busy = create(async () => clean); busy.prepare(); busy.state.busy = true;
  await busy.render().runSourceAudit();
  t('card generation prevents starting a source check', busy.calls.length === 0);
  const preparingBusy = create(async () => clean, { busy: true }); preparingBusy.render().prepareSourceAudit();
  ...
  for (const kind of ['notes', 'tier', 'source', 'batch']) {
    ...
    deferred.requests[0].resolve(clean); await completion;
  ...
  guardDeferred.requests[0].resolve(clean); await guardRun;
  ...
  cancelDeferred.requests[0].resolve(clean); await flush();
  ...
  cancelDeferred.requests[1].resolve(clean); await cancelRun;
  ...
  unmountDeferred.requests[0].resolve(clean); await unmountRun;
  ...
  const malformed = create(async () => ++malformedCount === 1 ? clean : 'MALFORMED_SYNTHETIC_RESPONSE');
  ...
  const truncated = create(async (_key, _model, _parts, options) => { options.onMeta({ truncated: true }); return clean; });
  ...
  const failedPreparation = create(async () => clean); failedPreparation.state.batch = { snapshot: { facts: [] }, chunkIds: [['missing-fact']] }; failedPreparation.render().prepareSourceAudit();
