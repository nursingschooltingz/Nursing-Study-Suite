...
The v15.17 reliability code is implemented. The example-only prompt candidate failed its live pilot and was rolled back with explicit user approval; the app again uses the exact pre-pilot prompt. Mapping correction and actual Anki import remain outstanding. The canonical app is `Nursing-Study-Suite v15.17.html`. On 2026-09-11, the user explicitly authorized publishing v15.17 as Latest. The [GitHub release](https://github.com/nursingschooltingz/Nursing-Study-Suite/releases/tag/v15.17) is published with the verified HTML attached, and the README links to that versioned download. Historical designs and prompt diffs are archived under `docs/history/`. Repository tooling requires exactly one suite HTML unless an explicit path is supplied; Proton Drive Name clash copies fail loudly.
