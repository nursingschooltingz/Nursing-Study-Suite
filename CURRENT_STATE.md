...
- **Validation:** 904 deterministic assertions, all frozen prompt hashes and generated documentation, LF/version checks, and full JSX Babel transform pass after the approved example edit. All eight SRI pins matched their CDNs on 2026-09-11. The synthetic browser fixture exercised edit/repair/manual exclusion, deletion, tier/style filtering, cloze switching, inert markup, warnings, source export, empty regeneration, stale batches, and late response rejection. The registry remained stable between changes and preserved other artifact kinds.
- **Generation settings are preserved.** A0 SHA was `21fb5980f796649d3cb7f53f2f919e8b93e2136b` with 761 assertions. All eleven frozen prompts stayed unchanged through the code-only checkpoint `1da58a3`; the subsequent approved example edit changes only `ANKI_MASTER_PROMPT`. Packet formatting, chunking, focus context, and the runtime mapping adapter retain their measured source hashes. Model/profile, transport, retry policy, and token ceilings were preserved.
- **Live measurement is outstanding; its KB is now scoped.** The user supplied a specific KB on 2026-09-11. Read-only planning found 40 conditions, 328 facts and eight chunks. A private before/after specification proposes 16 generation calls total, at most 48 attempts with the unchanged two-retry policy, and no extra audit calls. No live Gemini calls were made. The source path/hashes and pre-edit application snapshot are retained only in gitignored `scratch/anki-pilot/`; no other course files were inspected. The one-to-many adapter change has no evidence basis and was not applied.
- **Approved example edit applied on 2026-09-11.** The user's one-time approval covered exactly `ANKI-v15.17-example-proposal.diff`. Its five fictional complete examples now come from the shipped prompt in the structure, tags, style, numeric-fixture and collision assertions. Only the Anki baseline hash and its prompt documentation changed; the other ten frozen constants retain their baseline. Reversing the example diff recovers the exact prior Anki hash. This is deterministic evidence, not clinical validation or live-call authorization.
