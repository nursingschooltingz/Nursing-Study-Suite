...
Local v15.17 reliability code is implemented, but the approved example-only prompt candidate failed its live pilot and is not ready for publication. An exact rollback is prepared pending approval. The canonical app is `Nursing-Study-Suite v15.17.html`. No release was published or pushed during this implementation. Repository tooling now requires exactly one suite HTML unless an explicit path is supplied; Proton Drive Name clash copies fail loudly.
...
- **One-to-many mapping now has evidence, but the adapter is unchanged.** Three baseline notes legitimately split content from facts already mapped to other notes. Their missing edges support the conditional Stage D contract correction; they are not bad IDs, line numbers or parsing failures. Measure a proposed adapter change separately after resolving the failed example candidate and obtaining a further bounded live-call authorization.
- **The approved example edit remains applied pending rollback approval.** The user's one-time approval covered exactly `ANKI-v15.17-example-proposal.diff`. Deterministic formulation assertions pass, but the live pilot rejects adoption. `ANKI-v15.17-example-rollback.diff` is the exact inverse and restores the previous Anki hash; it has not been applied. Only the Anki baseline hash and its prompt documentation changed in the approved edit; the other ten frozen constants retain their baseline.
